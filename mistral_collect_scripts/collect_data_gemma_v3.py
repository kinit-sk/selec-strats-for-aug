...
default_prompt_baseline = """You are given a '{task}' classification dataset. Generate 5 new texts that fit the '{label}' category. Output each generated question in the form of a numbered list separated by new lines."""

default_qa_prompt_baseline = """You are given a '{task}' classification dataset. Generate 5 new questions that fit the '{label}' category. Output each generated question in the form of a numbered list separated by new lines."""

default_prompt_icl = """You will be given examples from '{task}' classification dataset, each labeled with a specific category. Based on the examples, generate 5 new texts that fit the '{label}' category. Output each generated question in the form of a numbered list separated by new lines.
Examples:
{examples}
"""

default_qa_prompt_icl = """You will be given examples of questions from '{task}' classification dataset, each labeled with a specific topic. Based on the examples of questions, generate 5 new questions that fit the '{label}' topic. Output each generated question in the form of a numbered list separated by new lines.
Examples:
{examples}
"""

default_mnli_prompt_baseline = """You will be given a premise with a label from a Natural Language Inference dataset. Generate 5 new hypotheses that '{label}' the given premise. The given premise: '{premise}'. Output each generated hypothesis in the form of a numbered list separated by new lines.
"""

default_mnli_prompt_icl = """You will be given a premise with a label from a Natural Language Inference dataset. Based on the examples, generate 5 new hypotheses that '{label}' the given premise. The given premise: '{premise}'. Output each generated hypothesis in the form of a numbered list separated by new lines.
Examples:
{examples}
"""
...
NO_SHOTS_PER_LABEL = 5 # 5
NO_ALL_SAMPLES_PER_LABEL = 20 # 20
...
if icl_strategy == 'baseline':
    for index, row in df_sub.iterrows():
        ...
        if dataset == 'yahoo' or dataset == 'trec':
            final_prompts.append((default_qa_prompt_baseline.format(task=dataset_to_task_dct[dataset], label=dataset_to_labels_dct[dataset][label]), label))
        elif dataset == 'mnli':
            final_prompts.append((default_mnli_prompt_baseline.format(premise=row['premise'], label=dataset_to_labels_dct[dataset][row['label']]), {'label': row['label'], 'premise': row['premise']}))
        else:
            final_prompts.append((default_prompt_baseline.format(task=dataset_to_task_dct[dataset], label=dataset_to_labels_dct[dataset][label]), label))

elif icl_strategy == 'synth_dis':
    df_0 = pd.read_csv(os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_gemma{folder_addiction}', str(random_state), 'baseline', 'uniform_para.csv'))
    for index, row in df_sub.iterrows():
        label = row['label']
        if dataset != 'mnli':
            text = row['text']
            ...
            rest_of_df = df_sub.drop(index)  # we dont use the case where premise might be included in the prompt, thats why we drop this
            rest_of_df['text'] = 'Premise: ' + rest_of_df['premise'] + ' Hypothesis: ' + rest_of_df['hypothesis']
            
        examples = get_synth_dis_examples_target_label_only(df_0, label, NO_SHOTS_PER_LABEL, sent_model)
        ...
        if dataset == 'yahoo' or dataset == 'trec':
            ...
            final_prompts.append((default_mnli_prompt_icl.format(premise=row['premise'], examples=str_examples, label=dataset_to_labels_dct[dataset][row['label']]), {'label': row['label'], 'premise': row['premise']}))
        else:
            final_prompts.append((default_prompt_baseline.format(task=dataset_to_task_dct[dataset], label=dataset_to_labels_dct[dataset][label]), label))
    ...
    for index, row in df_sub.iterrows():
        ...
        examples = get_similar_examples_gen(rest_of_df, label, NO_SHOTS_PER_LABEL, sent_model, random_state, method != 'uniform')
        ...
        if dataset == 'yahoo' or dataset == 'trec':
            final_prompts.append((default_qa_prompt_icl.format(task=dataset_to_task_dct[dataset], examples=str_examples, label=dataset_to_labels_dct[dataset][label]), label))
        elif dataset == 'mnli':
            final_prompts.append((default_mnli_prompt_icl.format(premise=row['premise'], examples=str_examples, label=dataset_to_labels_dct[dataset][row['label']]), {'label': row['label'], 'premise': row['premise']}))
        else:
            final_prompts.append((default_prompt_icl.format(task=dataset_to_task_dct[dataset], examples=str_examples, label=dataset_to_labels_dct[dataset][label]), label))
    ...
    for index, row in df_sub.iterrows():
        ...
        examples = get_diverse_examples_gen(rest_of_df, label, NO_SHOTS_PER_LABEL, sent_model, random_state, method != 'uniform')
    ...
    for index, row in df_sub.iterrows():
        ...
        if dataset != 'mnli':
            ...
            rest_of_df['text'] = rest_of_df['premise'] + ' : ' + rest_of_df['hypothesis']
        label = row['label']
        if method == 'uniform':
...
path_to_save = os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_gemma{folder_addiction}', str(random_state), icl_strategy)
if not os.path.exists(path_to_save):
    os.makedirs(path_to_save)

path_for_file = os.path.join(path_to_save, method+'_gen.pkl')
path_to_seeds_csv = os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_gemma{folder_addiction}', str(random_state), 'seeds.csv')
