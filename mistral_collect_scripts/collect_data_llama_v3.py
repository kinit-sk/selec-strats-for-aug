...
if icl_strategy == 'baseline':
    ...
    df_0 = pd.read_csv(os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_llama{folder_addiction}', str(random_state), 'baseline', 'uniform_para.csv'))
    for index, row in df_sub.iterrows():
        label = row['label']
        if dataset != 'mnli':
            text = row['text']
...
path_to_save = os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_llama{folder_addiction}', str(random_state), icl_strategy)
if not os.path.exists(path_to_save):
    os.makedirs(path_to_save)

path_for_file = os.path.join(path_to_save, method+'_gen.pkl')
path_to_seeds_csv = os.path.join('ICL_for_gen', 'datasets', dataset, f'collected_data_llama{folder_addiction}', str(random_state), 'seeds.csv')
